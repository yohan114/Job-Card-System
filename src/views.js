...
function outboxPage({ items }) {
  ...
  return `<div class="page-head"><h1>Email Outbox</h1><p class="muted">Vendor service-request emails sent by the system (simulated — see README to enable real SMTP).</p></div>
  ${items.length ? `<table class="table"><thead><tr><th>Subject</th><th>To</th><th>Job No.</th><th>Sent</th></tr></thead><tbody>${rows}</tbody></table>` : '<p class="muted">No emails sent yet.</p>'}`;
}

function outboxDetail({ mail }) {
  const att = (mail.attachments || []).map((a) => `<a href="${a.url}" target="_blank">📎 ${e(a.name)}</a>`).join(' ');
  return `<div class="page-head"><h1>${e(mail.subject)}</h1></div>
  <div class="panel email-view">
    ${kv('To', e(mail.to))}
    ${kv('Cc', e(mail.cc || '—'))}
    ${kv('Sent', fmtDateTime(mail.at))}
    ${kv('Attachments', att || '—')}
    <hr>
